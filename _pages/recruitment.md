...
# 香港科技大学（广州）DIGAI Lab招生详情


香港科技大学（广州）数据智能与几何人工智能实验室（DIGAI Lab) 诚邀有志于AI前沿研究的优秀学子加入，攻读博士学位或进行（远程）访问研究。实验室专注于表示学习、大模型、几何拓扑、信息检索，致力于推动几何与人工智能的理论发展，以及几何与人工智能在网络科学、科学发现领域的应用。欢迎具有不同学科背景的学生共同探索前沿领域，并提供丰富的科研资源与国际合作机会。
...
本研究方向致力于通过引入几何约束、几何引导和几何先验知识，系统性地提升大语言模型的Token嵌入质量和推理能力。我们的核心理念是利用数据和表示空间的内在几何结构，从根本上改善模型的表示学习和推理过程。

**核心研究领域包括：**

- **大模型表示分析(LLM Embedding)**：深入研究Transformer等大模型内部表示空间的潜在几何性质，探索高维嵌入空间中的拓扑结构、流形特征和几何不变性

- **多模态对齐（Multi-Modal LLMs）**：针对文本、图像、音频等不同模态数据，设计基于几何约束的统一表示框架，实现多源异构数据在几何结构上的有效对齐

- **强化学习推理与高效微调(Reasoning, PEFT)**：基于几何先验设计新的强化学习推理算法和微调策略，通过几何约束优化模型参数更新过程，提升推理准确性和训练效率
...
- **大规模网络科学的表示与优化(Network Science & Graph Learning)**：针对大规模复杂网络的建模、表示学习和优化问题，开发高效的算法和理论框架。重点研究社区发现、信息传播、影响力分析、链路预测等关键问题，结合图神经网络和几何深度学习技术，捕捉网络结构中的层次性和动态演化规律
...
本研究方向专注于将前沿人工智能技术与科学研究深度融合，通过基础模型和先进算法推动生物信息学、生物医药、材料科学等领域的变革性进展。我们致力于构建智能化的科学研究范式，加速科学发现的进程，为解决人类健康和可持续发展的重大挑战提供技术支撑。

**核心研究领域包括：**

- **大模型驱动的生物医药数据智能分析**：利用大语言模型和多模态模型处理生物医药领域的海量异构数据，包括基因序列分析、蛋白质结构预测、药物发现、临床数据挖掘等，实现从数据到知识的智能转化

- **几何机器学习在生命科学中的创新应用**：结合几何深度学习和生物信息学，开发针对分子结构、蛋白质折叠、药物-靶点相互作用等问题的几何约束模型，提升生物医药研究的精度和效率

我们欢迎对计算生物学、生物信息学、医学信息学、药物设计等交叉领域感兴趣的研究者加入我们的团队，共同推动AI4SCI领域的创新和突破，为科学研究的智能化发展贡献力量。
